...
const personaOptions = ['Budget-Conscious', 'Luxury-Oriented', 'Trend-Driven', 'NRI/Destination', 'Other'];
const aestheticOptions = ['Royal', 'Glam', 'Minimal', 'Rustic', 'Boho', 'Traditional', 'Modern', 'South Indian', 'Fusion'];
const cityOptions = ['Delhi', 'Jaipur', 'Chandigarh', 'Udaipur', 'Lucknow', 'Goa', 'Mumbai', 'Bangalore', 'Hyderabad', 'Chennai'];
const budgetOptions = ['₹3–10L', '₹10–50L', '₹50L–₹1Cr', '₹1Cr+'];
const weddingsManagedOptions = ['1–5', '6–15', '16–30', '30+'];
const guestSizeOptions = ['<100', '100–300', '300–700', '700+'];
const experienceOptions = ['<1 yr', '1–3 yrs', '3–5 yrs', '5–10 yrs', '10+ yrs'];
const teamSizeOptions = ['Solo', '2–5', '6–10', '10–20', '20–30', '30+'];


const BusinessProfile = forwardRef(({ onNext, isLastStep }: ComponentProps, ref) => {
  // State for all form fields
  const [clientPersona, setClientPersona] = useState('');
  const [aestheticStyles, setAestheticStyles] = useState<string[]>([]);
  const [baseLocation, setBaseLocation] = useState('');
  const [primaryCities, setPrimaryCities] = useState<string[]>([]);
  const [avgBudget, setAvgBudget] = useState('');
  const [weddingsManaged, setWeddingsManaged] = useState('');
  const [avgGuestSize, setAvgGuestSize] = useState('');
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select id="client-persona" value={clientPersona} onChange={(e) => setClientPersona(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md">
          ...
          <div>
            ...
            <input type="text" id="base-location" value={baseLocation} onChange={(e) => setBaseLocation(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md" />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap">
            {aestheticOptions.map(option => (
              <button key={option} onClick={() => handleToggle(option, aestheticStyles, setAestheticStyles)}
                className={`m-1 px-4 py-2 text-sm font-medium rounded-md transition-colors duration-200 ${aestheticStyles.includes(option) ? 'bg-black text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}>
                {option}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap">
            {cityOptions.map(option => (
              <button key={option} onClick={() => handleToggle(option, primaryCities, setPrimaryCities)}
                className={`m-1 px-4 py-2 text-sm font-medium rounded-md transition-colors duration-200 ${primaryCities.includes(option) ? 'bg-black text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}>
                {option}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <select id="avg-budget" value={avgBudget} onChange={(e) => setAvgBudget(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md">
          ...
          <div>
            ...
            <select id="weddings-managed" value={weddingsManaged} onChange={(e) => setWeddingsManaged(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md">
          ...
          <div>
            ...
            <select id="avg-guest-size" value={avgGuestSize} onChange={(e) => setAvgGuestSize(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <select id="experience" value={experience} onChange={(e) => setExperience(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md">
          ...
          <div>
            ...
            <select id="team-size" value={teamSize} onChange={(e) => setTeamSize(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-md">
