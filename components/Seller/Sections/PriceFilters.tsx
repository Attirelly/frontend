...
//           api.post(`/stores/store_types/price_ranges`, {
//             store_type_ids: storeTypes.map((st) => st.id),
//           }),
//         ]);
...
export default function PriceFiltersComponent() {
  ...
  const [maxPrice, setMaxPrice] = useState(priceFiltersData?.avgPriceMax?.toString() || '');
  const [storeTypes, setStoreTypes] = useState<StoreType[]>([]);
  const [storeTypePriceMap, setStoreTypePriceMap] = useState<Record<string, PriceRange[]>>({});
  const [selectedPrices, setSelectedPrices] = useState<StoreTypePriceRange[]>(priceFiltersData?.priceRanges || []);
    ...
    const fetchData = async () => {
      try {
        ...
        if (storeData?.store_types) {
          ...
          const priceRangeRes = await api.post(`/stores/store_types/price-range-ids`, {
  ...
  const handleSelect = (
    ...
    setSelectedPrices((prev) => {
      const updated = prev.filter((item) => item.store_type_id !== storeTypeId);
      return [...updated, { store_type_id: storeTypeId, store_type, price_range_id: priceRangeId, price_range }];
    });
  };
    ...
    <div className="p-6 rounded-2xl shadow-sm space-y-4 w-3xl bg-white">
      ...
      <div className="flex items-center gap-4">
        <div className="flex-1">
          ...
          <div className="flex gap-2">
            <div className="flex w-full border border-gray-300 rounded-md overflow-hidden">
              <span className="bg-gray-100 px-3 py-2 text-gray-500 select-none border-r border-gray-300">Rs</span>
              <input
                type="number"
                placeholder="2000"
                value={minPrice}
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      checked={isSelected}
                      onChange={() =>
                        handleSelect(storeType.id, price.id, storeType.store_type, price.label)
                      }
                      className="accent-black"
                    />
                    <div>
                      <div className="font-medium">{price.label}</div>
                      <div className="text-sm text-gray-600">
                        Rs {price.lower_value.toLocaleString()} - {price.upper_value.toLocaleString()}
                      </div>
                    </div>
                  </label>
                );
