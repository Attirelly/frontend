...
export default function SectionElevenContainer() {
  ...
  const handleNext = () => {
    if (scrollContainerRef.current) {
      // Scroll by the width of the container, creating a "page by page" scroll effect
  ...
  if (!stores || stores.length === 0) {
    return null; // Render nothing if there are no stores
  }
  ...
  return (
    <div className='w-full mx-auto space-y-4 lg:space-y-8'>
      ...
      <div className="relative flex items-center">
        ...
        {/* CHANGED: This is now a flex container that scrolls horizontally */}
        <div
          ref={scrollContainerRef}
          className="flex w-full space-x-4 overflow-x-auto snap-x snap-mandatory scroll-smooth scrollbar-none px-2"
        >
          {/* CHANGED: Map over the *entire* stores array */}
          {stores.map((card) => (
            // CHANGED: The child div now controls its own size responsively
            <div
              key={card.id}
              className="flex-shrink-0 w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/6 snap-start"
            >
