...
//   // --- No changes needed in your existing useEffects or data fetching ---
//   useEffect(() => {
//     const updateScreenSize = () => {
//       if (window.innerWidth < 768) setScreenSize("sm");
//       else if (window.innerWidth < 1024) setScreenSize("md");
//       else if (window.innerWidth < 1300) setScreenSize("lg");
...
//     if (products.length === 0) return [];
//     let range = 2;
//     if (screenSize === "sm") range = 0;
//     else if (screenSize === "md") range = 1;
//     const visibleIndices = [];
//     const totalCards = products.length;
//     for (let i = -range; i <= range; i++) {
//       let index = (centerIndex + i + totalCards) % totalCards;
//       visibleIndices.push(index);
//     }
//     return visibleIndices.map((index) => ({
//       ...products[index],
//       originalIndex: index,
//       offset: (index - centerIndex + totalCards + Math.floor(totalCards / 2)) % totalCards - Math.floor(totalCards / 2),
//     }));
//   }, [centerIndex, products, screenSize]);
...
//         <div className="relative flex items-center justify-center w-full h-[588px] ">
//           {isMobile ? (
//             // ✅ 2. RENDER LOGIC FOR MOBILE (WITH ANIMATION)
//             <AnimatePresence mode="wait">
//               {visibleCards.map((card) => (
//                 <motion.div
//                   key={card.id} // Key tells AnimatePresence which item is which
//                   className="w-[392px] h-[588px]"
//                   // Define enter, animate, and exit states
...
//             </AnimatePresence>
//           ) : (
//             // ✅ 3. ORIGINAL RENDER LOGIC FOR DESKTOP (STACKING EFFECT)
//             visibleCards
//               .sort((a, b) => a.originalIndex - b.originalIndex)
//               .map((card) => {
//                 const offset = card.offset;
...
//                 let heightClass = "";
//                 if (offset === 0) heightClass = "h-[588px]";
//                 else if (Math.abs(offset) === 1) heightClass = "h-[530px]";
//                 else heightClass = "h-[480px]";
...
//   return (
//     <div className="flex flex-col gap-4 md:gap-8 items-center w-full px-4 text-black">
