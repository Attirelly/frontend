...
const DynamicFilter = ({ context }: DynamicFilterProps) => {
  ...
  // Initialize localPriceRange when priceBounds changes
  useEffect(() => {
    if (context === 'product' && priceBounds) {
      setLocalPriceRange(priceBounds);
    }
  ...
  // Sync localPriceRange to global priceRange after slight delay
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (
        context === 'product' &&
    ...
    return () => clearTimeout(timeout);
  }, [localPriceRange, priceRange, context]);
...
export default React.memo(DynamicFilter);
