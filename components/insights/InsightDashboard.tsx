    ...
    <div className="flex items-center justify-between p-3 sm:p-4 border-b border-gray-200 last:border-b-0 hover:bg-gray-50 transition-colors duration-200 cursor-pointer">
      <div className="flex items-center space-x-3 sm:space-x-4">
        {/* <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gray-200 rounded-full flex-shrink-0"> */}
          ...
          width={12}
          height={12}
          className="w-12 h-12 sm:w-14 sm:h-14 bg-gray-200 rounded-full flex-shrink-0"/>
        {/* </div> */}
        <div>
...
const InstagramInsightsPage = () => {
  ...
  const [profileInsights, setProfileInsights] = useState<Profile[]>([]);
  const { setMediaInsights } = useCreatorStore(); 

  // --- CHANGE: Removed 'Followers Growth' ---
  const mainFilters = ["Engagement Rate", "Reach"];
  const timePeriods = ["1d", "7d", "28d"];

  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        const res = await api.get("/Creator/profiles");
        const profileData: Profile[] = res.data.map((item) => ({
          username: item.insta_username,
          profile_picture_url : item.profile_picture_url,
          followers: item.follower_count,
          engagement: item.engagement_rate,
          reach_day: item.reach_day,
          reach_week: item.reach_week,
          reach_month: item.reach_month,
          media: item.media,
        }));
        setProfileInsights(profileData);
      } catch (error) {
  ...
  const handleProfileClick = (profile: Profile) => {
    // 1. Set the media for the clicked profile in the Zustand store
    setMediaInsights(profile.media);
    // 2. Navigate to a new dynamic page for that creator
    router.push(`/insights/${profile.username}`);
  };
