...
export default function ProductUploadSideBar() {
    ...
    // Layout for Mobile (Horizontal)
    const MobileSidebar = () => {
        const scrollContainerRef = useRef<HTMLDivElement>(null);

        useEffect(() => {
            if (scrollContainerRef.current) {
                const activeButton = scrollContainerRef.current.querySelector<HTMLButtonElement>(`[data-section-id="${currentStep}"]`);
                if (activeButton) {
                    ...
                    {sections.map((section) => {
                        ...
                        return (
                            <button
                                ...
                                onClick={() => setCurrentStep(section.id)}
                                className={`flex items-center gap-2 p-2 rounded-lg transition min-w-[100px] ${isActive ? 'bg-blue-50' : 'bg-transparent hover:bg-gray-100'}`}
                            >
