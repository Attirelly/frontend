      ...
      >
        {croppingImage && (
          <>
            ...
            <button onClick={handleCrop} className="bg-blue-600 text-white px-4 py-2 rounded-lg w-full sm:w-auto sm:self-center">
              {currentFileIndex < filesToProcess.length - 1 ? "Save & Next" : "Save & Finish"}
            </button>
          </>
        )}
      </Modal>
      ...
      <div className="grid gap-6 mt-4">
        <div className="border border-gray-200 p-4 sm:p-6 rounded-xl">
          <h2 className="text-base sm:text-lg font-semibold mb-4 flex items-center gap-2">
            <FiImage className="text-gray-400" />
            Thumbnail Images
            {isUploading.main && (<span className="text-xs text-blue-500 ml-2">Uploading...</span>)}
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {mainPreviews.map((imageUrl, index) => (
              <div key={index} className="relative group aspect-[1/1.15]">
                <img src={imageUrl} alt={`Main product image ${index + 1}`} className="w-full h-full object-cover object-top rounded-lg border border-gray-200" />
                <button
                  onClick={(e) => { e.stopPropagation(); removeMainImage(imageUrl); }}
                  className="absolute top-1 right-1 bg-white p-1 rounded-full shadow-md hover:bg-gray-100 transition opacity-0 group-hover:opacity-100"
                  aria-label="Remove image"
                >
                  <FiTrash2 className="text-red-500 text-sm" />
        ...
        {variantsList.map((variant) => {
          ...
          const isUploadingVariant = isUploading.variants[sku];
          return (
            <div key={sku} className="border border-gray-200 rounded-xl p-4 sm:p-6">
              <h2 className="text-base sm:text-lg font-semibold mb-4 flex items-center gap-2">
                <FiImage className="text-gray-400" />
                Variant: {sku}
                {isUploadingVariant && (<span className="text-xs text-blue-500 ml-2">Uploading...</span>)}
              </h2>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {variantImages.map((imageUrl, index) => (
                  <div key={index} className="relative group aspect-[1/1.15]">
                    <img src={imageUrl} alt={`Variant ${sku} image ${index + 1}`} className="w-full h-full object-cover object-top rounded-lg border border-gray-200" />
                    <button
                      onClick={() => removeVariantImage(sku, imageUrl)}
                      className="absolute top-1 right-1 bg-white p-1 rounded-full shadow-md hover:bg-gray-100 transition opacity-0 group-hover:opacity-100"
                      aria-label="Remove image"
