...
export default function VariantAndInventory() {
  ...
  useEffect(() => {
    const isValid =
      variantsList.length > 0 &&
      variantsList.every(
        (v) => v.sku && v.size?.size_id && v.color?.color_id
      );
    setStepValidation(currentStep, isValid);
    ...
    selectedSizes.forEach((size) => {
      selectedColors.forEach((color) => {
        const isExcluded = excludedVariants.some(
          (ex) => ex.size_id === size.size_id && ex.color_id === color.color_id
        );
  ...
  const handleRemoveVariant = (size_id: string, color_id: string) => {
    setVariantsList((prev) =>
      prev.filter(
        (v) =>
          !(v.size.size_id === size_id && v.color.color_id === color_id)
      )
    );
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {variantsList.map((variant, index) => (
                <tr key={`${variant.size.size_id}-${variant.color.color_id}`}>
                  ...
                  <td className="px-4 py-2 whitespace-nowrap">{variant.size.size_name}</td>
                  <td className="px-4 py-2 whitespace-nowrap flex items-center gap-2">
                    <div
                      className="w-3 h-3 rounded-full border border-gray-300"
                      style={{ backgroundColor: variant.color.hex_code }}
