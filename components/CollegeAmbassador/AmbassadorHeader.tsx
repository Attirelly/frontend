...
export default function AmbassadorHeader() {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex gap-[24px] justify-center items-center">
          {res.map((item) => {
            ...
            return (
              <span
                className={`text-base selected cursor-pointer ${isSelected ? "text-black text-semibold" : "text-gray-400"
                  }`}
      ...
      {/* sidebar */}
      <div
        className={`fixed top-0 left-0 h-full w-64 bg-white p-4 flex flex-col shadow-lg transition-transform duration-300 z-50 ${hamburgerMenu ? 'transform translate-x-0' : 'transform -translate-x-full'
          }`}
      >
