"use client";
import { InstantSearch, SearchBox, Hits, RefinementList, Pagination, SortBy, Configure } from 'react-instantsearch';
...
type Seller = {
  ...
  outfits?: string[];
  genders?: string[];
  objectID?: string; // Required for Algolia
};
...
export default function Home() {
  ...
  function SellerHit({ hit }: { hit: Seller }) {
    return (
      <tr key={hit.id} className="hover:bg-gray-50">
        ...
        <td className="px-6 py-3 border">{hit?.store_types?.join(" , ")}</td>
        <td className="px-6 py-3 border">{hit?.outfits?.join(" , ")}</td>
        <td className="px-6 py-3 border">{hit?.genders?.join(" , ")}</td>
